...
import Dashboard, { Metric } from '@/components/ui/dashboard/Dashboard'
...
export default function AboutPageClient({ initialData }: Props) {
  // ————— Hook setup —————
  ...
  // Initialize word refs for the animated text
  ...
  if (wordRefs.current.length !== animatedTextLines.length) {
    wordRefs.current = animatedTextLines.map((line) =>
      Array(line.split(' ').length).fill(null)
    )
  }
  ...
  // ————— Static content —————
  const sponsors = [
    { id: 'fmp', name: 'FMP', logo: '/logo/fmp_logo.png', link: 'https://financialmodelingprep.com/' },
    { id: 'cair', name: 'CAIR', logo: '/logo/cair_logo.png', link: 'https://www.mun.ca/research/cair/' },
  ]
    ...
    <div className="about-page">
      ...
      <section className="sponsors-section">
        ...
        <div className="sponsors-container">
          {sponsors.map((s) => (
            <a key={s.id} href={s.link} target="_blank" rel="noreferrer" className="sponsor-logo-wrapper">
              <Image src={s.logo} alt={s.name} width={120} height={60} />
            </a>
          ))}
        </div>
      </section>
      ...
      <section className="marquee-section">
        ...
        <div className="marquee-container fadeout-horizontal">
          <div className="marquee-track">
            {[...advisors, ...advisors, ...advisors, ...advisors].map((adv, i) => (
              <div key={i} className="marquee-item">
                <Image src={adv.logo} alt={adv.alt} width={120} height={60} />
              </div>
            ))}
          </div>
        </div>
